...
class NetworkUtils(object):
    ...
    def cnn_layers(self, inputs, filters, kernel_size, strides):
        x = inputs
        for i in range(len(kernel_size)):
            with tf.variable_scope('unit-{}'.format(i + 1)):
                ...
                x = tf.keras.layers.BatchNormalization(axis=3, epsilon=1.001e-5, name='bn{}'.format(i + 1), fused=True)(x)
                x = tf.keras.layers.LeakyReLU(0.01)(x)
                x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=strides[i][1])(x)
        return x
    ...
    def conv_block(self, x, growth_rate, name):
        ...
        x1 = tf.keras.layers.Activation(activation='relu', name=name + '_0_relu')(x1)
        x1 = tf.keras.layers.Conv2D(
            filters=4 * growth_rate,
            kernel_size=1,
            use_bias=False,
            name=name + '_{}_conv'.format(i+1),
            dilation_rate=2 ** i,
            kernel_regularizer=l2(0.01),
        ...
        x1 = tf.keras.layers.Activation(activation='relu', name=name + '_1_relu')(x1)
